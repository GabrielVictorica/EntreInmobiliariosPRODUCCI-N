...
const Sidebar = React.memo(({ view, navigateTo, onLogout }: SidebarProps) => {
    ...
    // 2. Derive expanded group from view - Ready for first render
    // 4. Optimistic UI State (Instant Feedback)
    const [optimisticView, setOptimisticView] = useState<string | null>(null);
    const activeView = optimisticView || view;

    // Reset optimistic state when actual view updates
    React.useEffect(() => {
        if (view === optimisticView) {
    ...
    const derivedExpandedGroup = useMemo(() => {
        const v = activeView;
        if (['dashboard', 'form', 'properties-list', 'property-form'].includes(v)) return 'sellers';
        if (['buyer-clients-list', 'buyer-client-form', 'buyer-searches-list', 'buyer-search-form', 'visits-list', 'visit-form'].includes(v)) return 'buyers';
        if (['my-week', 'objectives', 'closings', 'habits'].includes(v)) return 'trakeo';
        if (['home', 'metrics-home', 'metrics-control'].includes(v)) return 'metrics';
        return null;
    }, [activeView]);

    // Internal state for manual toggles, initialized with null to respect derived group
    ...
    // 3. Stable click handlers

    const handleNav = useCallback((targetView: string) => {
        setOptimisticView(targetView); // Instant local update
        // We use requestAnimationFrame to allow the visual update to paint before the heavy render triggers
        requestAnimationFrame(() => {
            navigateTo(targetView);
        });
    }, [navigateTo]);

    // 5. Stable click handlers wrapped with optimistic logic
    const navToHome = useCallback(() => handleNav('metrics-home'), [handleNav]);
    const navToControl = useCallback(() => handleNav('metrics-control'), [handleNav]);
    const navToCalendar = useCallback(() => handleNav('calendar'), [handleNav]);
    const navToSellers = useCallback(() => handleNav('dashboard'), [handleNav]);
    const navToProperties = useCallback(() => handleNav('properties-list'), [handleNav]);
    const navToBuyers = useCallback(() => handleNav('buyer-clients-list'), [handleNav]);
    const navToSearches = useCallback(() => handleNav('buyer-searches-list'), [handleNav]);
    const navToVisits = useCallback(() => handleNav('visits-list'), [handleNav]);
    const navToWeek = useCallback(() => handleNav('my-week'), [handleNav]);
    const navToClosings = useCallback(() => handleNav('closings'), [handleNav]);
    const navToObjectives = useCallback(() => handleNav('objectives'), [handleNav]);
    const navToHabits = useCallback(() => handleNav('habits'), [handleNav]);

    return (
        <aside className="w-20 lg:w-64 flex-shrink-0 z-20 flex flex-col justify-between bg-[#364649] text-white transition-[width] duration-150 shadow-xl overflow-visible">
            <div>
                ...
                <nav className="mt-8 px-4 space-y-2">
                    ...
                    <div className="relative">
                        ...
                        <div className={`overflow-hidden transition-[max-height,opacity] duration-100 ${activeGroup === 'sellers' ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                    ...
                    <div className="relative">
                        ...
                        <div className={`overflow-hidden transition-[max-height,opacity] duration-100 ${activeGroup === 'buyers' ? 'max-h-48 opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className="ml-0 lg:ml-4 border-l border-white/10 pl-2 space-y-1 mt-1">
                                <NavItem
                    ...
                    <div className="relative">
                        ...
                        <div className={`overflow-hidden transition-[max-height,opacity] duration-100 ${activeGroup === 'trakeo' ? 'max-h-48 opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className="ml-0 lg:ml-4 border-l border-white/10 pl-2 space-y-1 mt-1">
                                <NavItem
                                    icon={icons.miSemana}
                                    label="Mí Semana"
