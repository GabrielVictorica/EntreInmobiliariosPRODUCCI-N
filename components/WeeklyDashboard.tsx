...
import { 
  ChevronLeft, ChevronRight, Plus, Users, Activity, UserPlus, Search, Link, X, ArrowRight, CheckCircle, Calendar, Trash2
} from 'lucide-react';
...
// Import Forms for Deep Integration
import ClientForm from './SellerForm';
import PropertyForm from './PropertyForm';
import BuyerClientForm from './BuyerClientForm';
import BuyerSearchForm from './BuyerForm';
import VisitForm from './VisitForm'; // NEW IMPORT
...
type WizardStep = 'select-contact' | 'create-contact' | 'create-record' | 'finalize';
...
const WeeklyDashboard: React.FC<WeeklyDashboardProps> = ({ 
  ...
  const startCreationFlow = (type: ActivityType) => {
      ...
      const isComplex = type === 'pre_buying' || type === 'pre_listing' || type === 'captacion' || type === 'visita';
  ...
  // 3. Finalize Activity
  const finalizeActivity = (contactId?: string, autoNotes?: string, refId?: string) => {
  ...
  const handleSimpleSubmit = (e: React.FormEvent) => {
      ...
      finalizeActivity(tempClientId || undefined);
  };
    ...
    <div className="space-y-6 pb-20 animate-fade-in-up">
      ...
      {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in-up">
              <div className={`bg-white rounded-3xl shadow-2xl w-full p-6 border border-white/20 transition-all duration-300 flex flex-col max-h-[90vh] ${modalMode === 'wizard' && wizardStep !== 'finalize' && wizardStep !== 'select-contact' ? 'max-w-5xl' : 'max-w-md'}`}>
                  ...
                  <div className="flex justify-between items-center mb-4 shrink-0">
                      <div>
                          <h3 className="text-lg font-bold text-[#364649] flex items-center">
                              ...
                              {modalMode === 'list' ? '' : (wizardStep === 'finalize' ? 'Registrar Actividad' : 'Gestión Vinculada')}
                          </h3>
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
                      ...
                      {/* MODE: LIST VIEW (Correction for Dead End UX) */}
                      {modalMode === 'list' && (
                                              ...
                                              <button 
                                                onClick={() => {
                                                    if(confirm('¿Eliminar esta actividad?')) {
                                                        ...
                                                        // Close modal if it was the last item
                      ...
                      {modalMode === 'wizard' && (
                          <>
                              ...
                              {wizardStep === 'select-contact' && (
                                  <div className="space-y-4">
                                      <p className="text-sm text-[#364649] bg-blue-50 p-3 rounded-lg border border-blue-100 flex items-start">
                                          ...
                                          Para mantener la base de datos limpia, primero selecciona o crea el cliente.
                                      </p>
                              ...
                              {wizardStep === 'create-contact' && (
                                  isBuyerFlow ? (
                                      <BuyerClientForm onSave={handleSaveNewBuyer} onCancel={() => setWizardStep('select-contact')} />
                                  ) : (
                                      <ClientForm onSave={handleSaveNewClient} onCancel={() => setWizardStep('select-contact')} />
                                  )
                              )}
