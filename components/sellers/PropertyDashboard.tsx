...
import { Building2, MapPin, Plus, Home, Edit3, ChevronDown, Sparkles, FileText, Calendar, Clock, Star, BarChart3, ShieldCheck, Search } from 'lucide-react';
...
const PropertyDashboard: React.FC<PropertyDashboardProps> = ({ onNewProperty, onEditProperty, onOpenMarketing }) => {
  const [expandedRow, setExpandedRow] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState('');

  const { properties, clients, visits, buyers, isLoading } = useBusinessStore(useShallow(state => ({
  ...
  const filteredProperties = useMemo(() => {
    ...
    return properties.filter(p =>
      ...
      p.address.neighborhood.toLowerCase().includes(lowerQuery) ||
      getOwnerName(p.clientId).toLowerCase().includes(lowerQuery)
    );
  }, [properties, searchQuery, clients]);
        ...
        {isLoading ? (
          <div className="p-16 text-center">
            <div className="flex flex-col items-center justify-center">
              ...
              <p className="text-[#364649]/60 font-medium">Cargando propiedades...</p>
            </div>
          </div>
          ...
          <div className="p-16 text-center">
            ...
            <h3 className="text-[#364649] font-bold text-lg">No hay propiedades encontradas</h3>
            <p className="text-[#364649]/60 text-sm mt-2">Intenta ajustar tu búsqueda o vincula una nueva propiedad.</p>
          </div>
        ) : (
