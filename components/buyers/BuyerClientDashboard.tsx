...
import { BuyerClientRecord, BuyerSearchRecord, VisitRecord, PropertyRecord } from '../../types';
...
interface BuyerClientDashboardProps {
  ...
  searches: BuyerSearchRecord[]; // Added prop
  visits: VisitRecord[]; // Added prop
  properties: PropertyRecord[]; // To resolve address in visits
  onNewClient: () => void;
...
const BuyerClientDashboard: React.FC<BuyerClientDashboardProps> = ({ clients, searches, visits, properties, onNewClient, onEditClient, onCreateSearch, onEditSearch }) => {
  ...
  return (
    <div className="space-y-8 pb-10">
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#364649] tracking-tight">Mis Compradores</h1>
          <div className="flex items-center gap-2 mt-1">
            <p className="text-[#364649]/60 text-sm font-medium">Base de contactos</p>
            <span className="w-1 h-1 rounded-full bg-[#364649]/30"></span>
            <p className="text-[#364649]/40 text-sm font-medium">{clients.length} personas</p>
          </div>
        </div>
        <button 
          onClick={onNewClient}
      ...
      <div className="bg-white/60 backdrop-blur-xl border border-white rounded-3xl overflow-hidden shadow-xl animate-fade-in-up">
        {clients.length === 0 ? (
          <div className="p-16 text-center">
             ...
             <h3 className="text-[#364649] font-bold text-lg">Sin clientes compradores</h3>
             <p className="text-[#364649]/60 text-sm mt-2">Agrega personas interesadas en comprar.</p>
          </div>
        ) : (
             ...
             <table className="min-w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-[#364649]/5">
                   {clients.map((c, i) => {
                      ...
                      const clientSearches = searches.filter(s => s.buyerClientId === c.id);
                      const clientVisits = visits.filter(v => v.buyerClientId === c.id);
...
export default BuyerClientDashboard;
