...
const VisitDashboard: React.FC<VisitDashboardProps> = ({ onNewVisit, onEditVisit }) => {
    const [searchQuery, setSearchQuery] = useState('');
    const { visits, properties, buyers, isLoading } = useBusinessStore(useShallow(state => ({
        visits: state.visits,
    ...
    const filteredVisits = React.useMemo(() => {
        ...
        return visits.filter(v => {
            ...
            return (
                ...
                p?.customId.toLowerCase().includes(lowerQuery) ||
                b?.name.toLowerCase().includes(lowerQuery)
            );
        });
    }, [visits, searchQuery, properties, buyers]);
            ...
            <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
                <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                        ...
                        <p className="text-[#364649]/40 text-sm font-medium">{filteredVisits.length} visitas encontradas</p>
                    </div>
                </div>

                <div className="flex-1 md:max-w-md mx-auto w-full">
                    <div className="relative group">
                        <List className="absolute left-3 top-1/2 -translate-y-1/2 text-[#364649]/40 group-focus-within:text-[#AA895F] transition-colors" size={18} />
                        <DebouncedInput
                            type="text"
                            placeholder="Buscar por propiedad o cliente..."
                            value={searchQuery}
                            onChange={(val) => setSearchQuery(val as string)}
                            className="w-full bg-white/50 border border-[#364649]/10 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#AA895F]/20 transition-all"
                        />
                    </div>
                </div>
                ...
                {isLoading ? (
                    <div className="p-16 text-center">
                        <div className="flex flex-col items-center justify-center">
                            ...
                            <p className="text-[#364649]/60 font-medium">Cargando agenda...</p>
                        </div>
                    </div>
                    ...
                    <div className="p-16 text-center">
                        ...
                        <h3 className="text-[#364649] font-bold text-lg">No hay visitas encontradas</h3>
                        <p className="text-[#364649]/60 text-sm mt-2">Intenta ajustar tu búsqueda o registra una nueva visita.</p>
                    </div>
                ) : viewMode === 'list' ? (
                    // --- LIST VIEW ---
...
export default React.memo(VisitDashboard);
