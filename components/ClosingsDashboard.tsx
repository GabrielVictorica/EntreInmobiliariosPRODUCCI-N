...
const ClosingsDashboard: React.FC<ClosingsDashboardProps> = ({ closings, activities, properties, clients, buyers, onAddClosing }) => {
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [commissionSplit, setCommissionSplit] = useState(45); 
  
  // Hardcoded Exchange Rate for Prototype (Ideally fetched from an API)
  const EXCHANGE_RATE_ARS_TO_USD = 1000; 
  ...
  const normalizeToUSD = (amount: number, currency: string) => {
      ...
      if (currency === 'ARS') return amount / EXCHANGE_RATE_ARS_TO_USD;
      return amount;
  };
  ...
  return (
    <div className="space-y-8 pb-10 animate-fade-in-up">
      ...
      {/* 3. CLOSINGS TABLE */}
      <div className="bg-white rounded-3xl border border-[#364649]/10 overflow-hidden shadow-xl animate-fade-in-up" style={{animationDelay:'0.2s'}}>
          <div className="p-6 border-b border-[#364649]/5 bg-gray-50/50 flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                  <div className="text-[10px] text-[#364649]/40 italic">
                      * Totales convertidos a USD (TC aprox. {EXCHANGE_RATE_ARS_TO_USD})
                  </div>
